<div class="div-principal">
    <div class="max-w mx-auto sm:px-6 lg:px-8"
        ...
        x-init="@this.on('show-toast', (data) => showToast(data))" 
        x-cloak>
        <div class="md:p-8">
            ...
            <header class="mb-8 border-b dark:border-gray-700 pb-4 flex flex-col sm:flex-row justify-between sm:items-center space-y-3 sm:space-y-0">
                <div>
                    <h1 class="titles">
                        Arma tu Rutina
                    </h1>
                ...
                <div class="w-full sm:w-auto">
                    @if ($currentView === 'selector')
                        <button wire:click="changeView('routine')"
                            class="w-full sm:w-auto btn-ouline-ve"
                                    ...
                                    @foreach ($groups as $group)
                                        ...
                                        @if ($exercisesInGroup->isNotEmpty())
                                            @foreach ($exercisesInGroup as $exercise)
                                                <div class="flex items-center justify-between p-2 rounded-lg transition duration-150 flex-nowrap
                                                    ...
                                                    <div class="flex items-center space-x-3 flex-grow min-w-0">
                                                        ...
                                                        <div class="flex items-start space-x-2 flex-grow min-w-0">
                                                            ...
                                                            <label for="exercise-{{ $exercise->id }}" 
                                                                ...
                                                                <span class="block text-xs font-normal text-gray-500 dark:text-gray-400 capitalize mt-0.5">
                                                                    {{ Str::ucfirst($group) }}
                                                                </span>
                                                            </label>
                                                        </div>
                                    ...
                                    <div class="card-tb-ve-v2">
                                        <div class="flex justify-between items-center w-full p-2">
                                            ...
                                            <div class="flex flex-col min-w-0 pr-4">
                                                <div class="text-lg font-bold text-lime-600 dark:text-lime-400 truncate">
                                                    {{ $exercise->name }}
                                                </div>
                                                <div class="text-sm text-gray-500 dark:text-gray-400 capitalize truncate">
                                        ...
                                        <div class="mt-4 border-t pt-4 dark:border-gray-700 space-y-3">
                                            ...
                                            @forelse ($routineData[$exercise->id] ?? [] as $setIndex => $set)
                                                <div class="flex flex-col w-full p-3 rounded-lg border border-gray-700/50 bg-[#262627] shadow-lg transition duration-300 ease-in-out hover:border-lime-500/50">

                                                    <div class="flex items-center space-x-2 w-full">
                                                        
                                                        <span class="font-bold text-base text-lime-400 whitespace-nowrap shrink-0 pr-1">
                                                            Set {{ $setIndex + 1 }}
                                                        </span>
                                                        
                                                        <div class="flex items-center space-x-2 flex-1 min-w-0"> 
                                                            
                                                            <div class="flex items-center flex-1"> <label for="reps-{{ $exercise->id }}-{{ $setIndex }}" class="text-xs font-medium text-gray-300 mr-1 whitespace-nowrap">Reps:</label> 
                                                                <flux:input
                                                            ...
                                                            <div class="flex items-center flex-1"> <label for="kg-{{ $exercise->id }}-{{ $setIndex }}" class="text-xs font-medium text-gray-300 mr-1 whitespace-nowrap">KG:</label> 
                                                                <flux:input
                                            ...
                                            @endforelse
                                                ...
                                                {{-- <button wire:click="addSet({{ $exercise->id }})"
                                                    class=" btn-ouline-ve w-full mt-2">
                                                    + Añadir Set
                                                </button> --}}
                                        </div>
                                        ...
                                        <div class="mt-3 flex space-x-2 w-full">
                                            <button wire:click="addSet({{ $exercise->id }})" 
                                                class="flex-1 py-1 btn-ouline-ve">
                                                + Añadir Set
                                            </button>
                                            <button wire:click="toggleExercise({{ $exercise->id }})"
                                                class="flex-1 py-1  btn-ouline-ro">
                                                Quitar Ejercicio
                                            </button>
                                        </div>
                                    </div>
                ...
                >
                    ...
                    >
                        <div class="border-b border-gray-700 pb-4 mb-4 flex items-center justify-between">
                            ...
                            <button @click="open = false" class="text-gray-400 hover:text-white transition duration-200">
        ...
        <div x-show="toast.show"
            ...
            @click="toast.show = false" {{-- Clic para cerrar --}}
            ...
            <div class="max-w-xs w-full text-white p-4 rounded-lg shadow-xl font-semibold transform transition duration-300 cursor-pointer flex items-center space-x-3">
                ...
                <svg x-show="toast.type === 'error' || toast.type === 'warning'" class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
