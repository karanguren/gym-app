                ...
                @forelse ($trainers as $trainerItem)
                    <div class="card-tb-ve-v2 !p-6">
                        ...
                        <div class="mt-4 flex space-x-2">
                            @if ($status === 'accepted' || $status === 'pending' || $status === 'rejected')
                                <button @click="openDetachModal()" class="w-1/2 mt-2 sm:mt-0 btn-outline-ro">
                                    @if ($status === 'pending')
                                        Cancelar Solicitud
                                    @else
                                        Desvincular Entrenador
                            ...
                            <button wire:click="openSelectionModal({{ $trainerItem->id }}, '{{ $trainerItem->name }}')"
                                ...
                                class="w-1/2 
                                    @if ($status === 'pending' || $trainerItem->id === $assignedTrainer?->id) btn-outline-grey-disabled
                                    @else
                                        btn-outline-ve @endif">
                                @if ($trainerItem->id === $assignedTrainer?->id)
                                    Actual Entrenador
                    ...
                    <div x-show="show" x-transition:enter="ease-out duration-300"
                        ...
                        <div class="card-tb-ve-v2">
                            ...
                            <div class="flex justify-end space-x-3">
                                <button wire:click="closeSelectionModal" type="button" class="btn-outline-ro">
                                    Cancelar
                                </button>
                                <button wire:click="selectTrainer" type="button" class="btn-outline-ve">
                                    Enviar Solicitud
                                </button>
                            </div>
                        </div>
            ...
            <div x-data="{ show: @entangle('showDetachModal') }" x-show="show" x-cloak class="fixed inset-0 z-50 overflow-y-auto"
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div x-show="show" x-transition:enter="ease-out duration-300"
                        ...
                        <div class="card-tb-ve-v2">
                            <svg class="mx-auto h-12 w-12 text-red-600 dark:text-red-400"
                                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 mb-6 text-center">
                                ¿Estás seguro de que deseas desvincularte de tu entrenador actual? Esta acción cancelará
                                tu
                                solicitud pendiente o removerá tu asignación aceptada.
                            </p>

                            <div class="flex justify-end space-x-3">
                                <button wire:click="closeDetachModal" type="button" class="btn-outline-ve">
                                    No, Cancelar
                                </button>
                                <button wire:click="detachTrainer" type="button" class="btn-outline-ro">
                                    Sí, Desvincular
                                </button>
                            </div>
                        </div>
